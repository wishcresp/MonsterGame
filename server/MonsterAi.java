...
public class MonsterAi extends Graph 
{
	...
	public void calculate_shortest_path() 
	{
		...
		// WHEN THE MONSTER TOUCHES THE PLAYER CALL player.kill
		
		// This shits itself when the player touches the monster gets an array out of bounds -1
		...
		for (int i = 0; i < this.vertex_array.length; i++) 
		{
			...
			for (int x = 0; x < amount_of_edges.size(); x++) 
			{
				...
				if (!this.vertex_array[neighbour].is_settled()) 
				{
					...
					{						
						/*
						 * TODO for each time the algorithm makes its way towards the target, build a
	...
	private int get_shortest_distance() 
	{
		...
		for (int i = 0; i < this.vertex_array.length; i++) 
		{
			...
			// If current vertex is not settled and distance is lowest
