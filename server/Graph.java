...
public class Graph 
{
	// GRAPH VARIABLES //
	/*
	 * Vertex array manages the 'settled' boolean as well as the distance from the
	 * source vertex
	 * 
	...
	/*
	 * Manages the connection between each vertex and the length of each edge, the
	 * default value will be set to 1 as this is not a weighted network.
	 * 
	...
	/*
	 * Constructor will take in an array of Edges and build the graph by building
	...
	public Graph(GraphEdge[] edge_array) 
	{
		...
		/*
		 * Create new array based on the amount of vertices there are. If there are 8
		 * nodes, create an array of length 8 (0-7)
		 */
		this.vertex_array = new GraphVertex[this.no_of_vertices];

		/*
		 * All the arrays will start off as null from creation, loop through each and
		...
		/*
		 * The index 'x' focuses on the current vertex, and represents the amount of
		 * edges to add
		 */
	...
	public void print_result(int source_node) 
	{
		String output = "";
		/*
		 * output = "\nNumber of vertices = " + this.no_of_vertices; output +=
		 * "\nNumber of edges = " + this.no_of_edges + "\n";
		 */
		for (int i = 0; i < this.vertex_array.length; i++) {
			output += ("\nDistance from vertex " + source_node + " to vertex " + i + " is "
					+ vertex_array[i].get_distance_from_source());
